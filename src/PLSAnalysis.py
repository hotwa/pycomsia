...
class PLSAnalysis:
    ...
    def convert_fields_to_X(self, train_fields, pred_fields=None, filter=0.0):
        """Convert fields to X matrix with filtering and scaling across all molecules for each field"""
        n_train_molecules = len(train_fields['steric_field'])
        X_train_raw = []
        for mol_idx in range(n_train_molecules):
            mol_features = []
            for field_name in train_fields:
                mol_features.extend(train_fields[field_name][mol_idx])
            X_train_raw.append(mol_features)
        X_train_raw = np.array(X_train_raw)
        df = pd.DataFrame(X_train_raw)
        df.to_csv("fields_output.csv", index=False)
        
        
        # 1. Combine training and prediction sets
        combined_fields = {}
        n_train_molecules = len(train_fields['steric_field'])
        ...
        # 2. Filter columns while tracking indices
        self.kept_indices = {}
        ...
        for field_name in combined_fields:
         ...
         # 3. Split back into train/pred sets and scale
        X_train = []
        X_pred = None if pred_fields is None else []
        self.scalers = {}
        
        for field_name in filtered_fields:
            field_data = filtered_fields[field_name]
            
            # Split back into train/pred
            train_portion = field_data[:n_train_molecules]
            pred_portion = field_data[n_train_molecules:] if pred_fields is not None else None
            
            # Scale all values for this field
            self.scalers[field_name] = StandardScaler()
            
            # Transform train and pred separately
            scaled_train = self.scalers[field_name].fit_transform(train_portion.reshape(-1, 1))
            scaled_train = scaled_train.reshape(train_portion.shape)
            self.field_stdevs[field_name] = np.std(scaled_train)
            
            if pred_portion is not None:
                scaled_pred = self.scalers[field_name].fit_transform(pred_portion.reshape(-1, 1))
                scaled_pred = scaled_pred.reshape(pred_portion.shape)
            else:
                scaled_pred = None
            ...
            if pred_fields is not None:
                if len(X_pred) == 0:
                    ...
                    X_pred = np.hstack((X_pred, scaled_pred))

        # 4. Mean center by column
        pd.DataFrame(X_train).to_csv("scaled_fields_output.csv", index=False)
        X_train_means = np.mean(X_train, axis=0)
        X_train = X_train - X_train_means
        
        if X_pred is not None:
            X_pred = X_pred - X_train_means
    ...
    def convert_fields_to_X(self, train_fields, pred_fields=None, filter=0.0):
        ...
        for field_name in filtered_fields:
            ...
            scaled_pred = scaled_data[n_train_molecules:] if pred_fields is not None else None
        ...
        if X_pred is not None:
            X_pred_means = np.mean(X_pred, axis=0)
            X_pred = X_pred - X_pred_means
    ...
    def fit_final_model(self, train_activities, test_size=0.2, predict_smiles_list=None):
        """
        Split data into train/test, fit final model with optimal components on training set,
        ...
        self.X_train_final = X_train_split
        self.X_test_final = X_test_split
        self.y_train_final = y_train_split
        self.y_test_final = y_test_split
        ...
        self.pls_model.fit(X_train_split, y_train_split)

        # Get predictions for both sets
        self.y_train_predicted = self.pls_model.predict(X_train_split).ravel()
        self.y_test_predicted = self.pls_model.predict(X_test_split).ravel()

        # Reverse mean-centering for predictions and test values
        self.y_train_final_original = self.y_train_final + y_mean
        self.y_train_predicted_original = self.y_train_predicted + y_mean
        self.y_test_predicted_original = self.y_test_predicted + y_mean
        self.y_test_final_original = self.y_test_final + y_mean
        ...
        # Make predictions for unseen molecules if prediction set exists
        if self.X_pred is not None:
            self.y_pred_unseen = self.pls_model.predict(self.X_pred).ravel()
            self.y_pred_unseen_original = self.y_pred_unseen + y_mean  # Reverse mean-centering for unseen predictions
            df = pd.DataFrame({
                "SMILES": predict_smiles_list,
                "Predicted Activity": self.y_pred_unseen_original
            })
    ...
    def plot_results(self, outputdir):
        ...
        plt.text(0.95, 0.05, f'R²(train) = {self.r2_train:.4f}',
            transform=plt.gca().transAxes,
            bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
            horizontalalignment='right')  # Align text to the right

        plt.text(0.95, 0.12, f'R²(test) = {self.r2_test:.4f}',
            transform=plt.gca().transAxes,
            bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
